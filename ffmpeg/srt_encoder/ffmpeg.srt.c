...
#define RTT_LIST_MAX    6
#define BW_LIST_MAX     6
#define STATE_LIST_MAX  6
#define CWND_GAIN_DEF   1.3
#define RTT_INIT        100
#define RTT_MIN         35
...
static int g_srtlog = 1;
static int g_is_srt = 0;
static int64_t s_last_srt_ts = 0;
static const int64_t SRT_CHECK_INTERVAL = 300;

static int s_rtt_array[RTT_LIST_MAX];
static int s_rtt_index;
static int s_rtt_min;//ms
static int s_rtt_ready;

static unsigned long long s_bw_array[BW_LIST_MAX];//bits/s
static int s_bw_index;
static unsigned long long s_bw_max;//bits/s, max srt bandwidth
static unsigned long long s_avg_bw;//bits/s, avg srt bandwidth
static int s_bw_ready;

static int64_t s_state_array[STATE_LIST_MAX];
static int s_state_index;
static int s_state_ready;

static double s_cwnd_gain;
static int s_current_state;
...
static URLContext *srt_ffio_geturlcontext(AVIOContext *s);
...
static void srt_log(const char* fmt, ...) {
    char buffer[2048];
    va_list       ap;
    FILE* file_p;
    ...
    file_p = fopen("srt.log", "ab+");
    if (file_p) {
        fwrite(buffer, 1, strlen(buffer), file_p);
        fclose(file_p);
    }
}
...
static void srt_congestion_init(void) {
    memset(s_rtt_array, 0, sizeof(s_rtt_array));
    s_rtt_index = 0;
    s_rtt_min = RTT_INIT;
    s_rtt_ready = 0;

    memset(s_bw_array, 0, sizeof(s_bw_array));
    s_bw_index = 0;
    s_bw_max = 0;
    s_bw_ready = 0;

    memset(s_state_array, 0, sizeof(s_state_array));
    s_state_index = 0;
    s_state_ready = 0;

    s_cwnd_gain = CWND_GAIN_DEF;
    s_current_state = STATE_KEEP;

    s_congestion_state = STATE_KEEP;
    s_video_bitrate = 0;
    s_last_video_perf_ts = 0;
    s_enc_ctx = NULL;
}

    //update current rtt in ms
static void update_rtt(int rtt) {
    if (rtt < RTT_MIN) {
        rtt = RTT_MIN;
    }
    s_rtt_array[s_rtt_index] = rtt;
    s_rtt_index = (s_rtt_index + 1)%RTT_LIST_MAX;
    if (s_rtt_index == 0) {
        s_rtt_ready = 1;
    }
    if (s_rtt_ready) {
        s_rtt_min = s_rtt_array[0];
        for (int index = 0; index < RTT_LIST_MAX; index++) {
            av_log(NULL, AV_LOG_DEBUG, "update rtt[%d]=%d, input rtt:%d.\r\n", 
                index, s_rtt_array[index], rtt);
            if ((s_rtt_min == 0) || (s_rtt_min > s_rtt_array[index])) {
                s_rtt_min = s_rtt_array[index];
            }
        }
    }
}
...
static void update_maxbw(unsigned long long maxbw) {
    int64_t total;

    s_bw_array[s_bw_index] = maxbw;
    s_bw_index = (s_bw_index + 1)%BW_LIST_MAX;
    if (s_bw_index == 0) {
       s_bw_ready = 1;
    }
    if (s_bw_ready) {
        s_bw_max = s_bw_array[0];
        total = 0;
        for (int index = 0; index < BW_LIST_MAX; index++) {
            total += s_bw_array[index];
            if ((s_bw_max == 0) || (s_bw_max < s_bw_array[index])) {
                s_bw_max = s_bw_array[index];
            }
        }
...
static int srt_bitrate_get_state(int inflight) {
    ...
    bdp = s_bw_max*s_rtt_min/1000;
    inflight = inflight*8;
    current_state = s_cwnd_gain * bdp - inflight;

    s_state_array[s_state_index] = current_state;
    s_state_index = (s_state_index+1)%STATE_LIST_MAX;
    if (s_state_index == 0) {
        s_state_ready = 1;
    }
    if (!s_state_ready) {
...
static int update_srt_enable(const char* url) {
    char* p = strstr(url, "srt://");
    if (p != url) {
        g_is_srt = 0;
        return 0;
    }
...
static void congestion_ctrl(AVFormatContext *s) {
    ...
    urlctx = (URLContext*)srt_ffio_geturlcontext(s->pb);
    srtctx = urlctx->priv_data;
    ...
    if (ret != 0) {
        av_log(NULL, AV_LOG_DEBUG,"congestion srt_bstats retrun %d", ret);
        return;
    }
...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost, int unqueue)
{
    ...
    congestion_ctrl(s);//added for srt adaptive bitrate encoder
                         ...
                         AVFrame *next_picture)
{
    ...
    for (i = 0; i < nb_frames; i++) {
        ...
        reset_vencode_bitrate();//added for srt adaptive bitrate encoder 
                              ...
                              char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if ((ost->enc->type == AVMEDIA_TYPE_VIDEO) && ((ost->enc_ctx->codec_id == AV_CODEC_ID_H264)
            || (ost->enc_ctx->codec_id == AV_CODEC_ID_H265))) {
            s_video_bitrate = ost->enc_ctx->bit_rate;
            srt_log("init srt video encoder bitrate:%ld.\r\n", s_video_bitrate);
            s_enc_ctx = ost->enc_ctx;
        }
    ...
    ost->initialized = 1;
    //srt_init
    update_srt_enable(output_files[ost->file_index]->ctx->url);
